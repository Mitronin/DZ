...
from .forms import AuthForm, RegistrationForm, AddBookForm
...
def page_request(request, page_id):
    context = {
        'books' : ObjectListView.get_page_dict(page_id),
    }
...
class ObjectListView(BaseView): 
    ...
    def get_page_dict(page_id):
        page_id = int(page_id)
        st_pos = len(Book.objects.all()) - ObjectListView.objOnList * page_id
        ...
        all = Book.objects.all()[start: end][::-1]
    ...
    def get(self, request):
        return super().get(
            ...
            context = {
                'name' : 'Books',
                'books': ObjectListView.get_page_dict(0),
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            if f is None:
                ...
                file_url = r'images/pokemons/%d%s' % (book.id, '.jpg')
                filename = FileSystemStorage().save('main/static/' + file_url, File(f))
            
            book.imageUrl = file_url
            book.save()
            return redirect('single_object', book_id=book.id)
...
class ObjectView(BaseView):
    def get(self, request, book_id):
        ...
        context = {
            'book' : obj,
            'status' : obj.orders.filter(id=request.user.id).exists()
        }
    ...
    def post(self, request, book_id):
        ...
        if request.user.is_authenticated():
            ...
            rel = book.orders.filter(id=request.user.id).exists()
            if state == 'True' and not rel:
                    book.orders.add(request.user)
            
            if state == 'False' and rel:
                    book.orders.remove(request.user)
        
        return render_to_response(
            'object/base_user_list.html', 
            { 'users' : book.orders.all() }
        )
