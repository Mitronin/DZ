...
$('#add_rec_button').click(function(){
    ...
    var name = form.find('#new_rec_name').val(),
        author = form.find('#new_rec_country').val(),
        genre = form.find('#new_rec_sportType').val(),
        desc = form.find('#new_rec_desc').val();

    if (name.length == 0) {
        userMessage.show('Введите название книги.');      
        return;
    }

    if (author.length == 0) {
        userMessage.show('Введите автора.');      
        return;
    }

    if (genre.length == 0) {
        userMessage.show('Введите жанр.');      
        return;
    }

    if (desc.length == 0) {
        userMessage.show('Введите описание книги.');      
        return;
    }

    var input = document.getElementById('new_rec_img');
    if (!input.files || !input.files[0] || input.files[0].size == 0) {
        userMessage.show('Добавьте обложку для книги.');      
        return;
    }
